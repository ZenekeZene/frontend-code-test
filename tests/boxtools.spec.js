...
test.describe('Interacting with the tools of each box: ', () => {
	test(`It allows the user to double click on a box and insert text`, async ({ page }) => {
	...
	test(`It allows the user to change the background color of a box`, async ({ page }) => {
		const boxes = page.getByRole('button', { name: 'Draggable box' });
		const firstBox = boxes.first();
		await firstBox.click();
		await expect(firstBox).toHaveAttribute('aria-pressed', 'true');
	...
	test(`It allows the user to change the text color of a box`, async ({ page }) => {
		const boxes = page.getByRole('button', { name: 'Draggable box' });
		...
		const style = await secondBox.getByRole('textbox', { name: /Edit the text/ }).getAttribute('style');
	...
	test(`It allows the user to edit the text of a box`, async ({ page }) => {
		...
		const editTextButton = firstBox.getByRole('menuitem', { name: 'edit text' });
