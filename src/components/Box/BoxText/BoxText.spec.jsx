...
describe('BoxText component:', () => {
	test('if is in editing mode, the user can type on it', async () => {
		...
		const boxTextAfterTyping = screen.getByRole('textbox', { name: "Edit the text" });

		expect(boxTextAfterTyping).toHaveTextContent('Hello, this is my first day at Genially.');
	});
	...
	test('if is not in editing mode, the user cannot type on it', async () => {
		...
		expect(boxTextAfterTyping).toHaveTextContent('I would like to write but...');
	});
});
